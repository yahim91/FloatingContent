...
netconvert --type-files typemap.xml \
  ...
  --geometry.remove --remove-edges.isolated --tls.join --verbose --osm-files $1 --output-file $network


echo "Creating route file for $1"

~/sumo/tools/trip/randomTrips.py -n $network --min-distance 1000 -e 3600 -o $trips
