...
received information for. Thus, when a peer is dicovered i.e. a beacon is
...
vehicle during its soujorn time, etc. We decided to hold this data inside the
...
the vehicle, the box within the vehicle resides from the grid can be determined.
Each of the corners represents the center of an anchor zone. If the radius of an
anchor zone would be equal with the distance between the anchor zones, the
vehicle can reside in some or all the anchor zones placed in the corners of its
box as shown in Figure ~\ref{fig:node_az}.

\begin{figure}[t]
	\centering
	\includegraphics[width=0.4\textwidth]{img/node_az}
	\caption{Anchor zones in which node 8 resides, are coloured with red, the
	other ones are coloured with yellow}
	\label{fig:node_az}
\end{figure}

%\begin{figure}[t]
%	\centering
%	\includegraphics[width=0.6\textwidth]{img/SanFrancisco_Anchors}
%	\caption{Simulation area and anchor locations every 200m}
%	\label{fig:SanFrancisco_Anchors}
%\end{figure}