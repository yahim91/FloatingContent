...
std::string TraCIScenarioManager::receiveTraCIMessage() {
    if (!socketPtr)
        error("Not connected to TraCI server");
    ...
    uint32_t msgLength;
    {
        ...
        while (bytesRead < sizeof(uint32_t)) {
            int receivedBytes = ::recv(MYSOCKET,
...
void TraCIScenarioManager::sendTraCIMessage(std::string buf) {
...

    {
        ...
        while (bytesWritten < buf.length()) {
            size_t sentBytes = ::send(MYSOCKET, buf.c_str() + bytesWritten,
                    buf.length() - bytesWritten, 0);
            if (sentBytes > 0) {
                bytesWritten += sentBytes;
            } else {
                if (sock_errno() == EINTR)
...
std::string TraCIScenarioManager::makeTraCICommand(uint8_t commandId,
        TraCIBuffer buf) {
...
TraCIScenarioManager::TraCIBuffer TraCIScenarioManager::queryTraCI(
        uint8_t commandId, const TraCIBuffer& buf) {
    ...
    if (result == RTYPE_ERR)
        error("TraCI server reported error executing command 0x%2x (\"%s\").",
                commandId, description.c_str());
    ASSERT(result == RTYPE_OK);
    return obuf;
}

TraCIScenarioManager::TraCIBuffer TraCIScenarioManager::queryTraCIOptional(
        uint8_t commandId, const TraCIBuffer& buf, bool& success,
        std::string* errorMsg) {
    sendTraCIMessage(makeTraCICommand(commandId, buf));
...
void TraCIScenarioManager::connect() {
    ...
    {
        int x = 1;
        ::setsockopt(MYSOCKET, IPPROTO_TCP, TCP_NODELAY, (const char*) &x,
                sizeof(x));
    }
}
...
void TraCIScenarioManager::init_traci() {
    {
        std::pair<uint32_t, std::string> version =
                TraCIScenarioManager::commandGetVersion();
        ...
        if ((apiVersion == 3) || (apiVersion == 5) || (apiVersion == 6)
            ...
            error(
                    "TraCI server \"%s\" reports API version %d. This server is unsupported.",
...
std::pair<uint32_t, std::string> TraCIScenarioManager::commandGetVersion() {
    bool success = false;
    TraCIBuffer buf = queryTraCIOptional(CMD_GETVERSION, TraCIBuffer(),
            success);
...
void TraCIScenarioManager::commandSetSpeedMode(std::string nodeId,
        int32_t bitset) {
...
void TraCIScenarioManager::commandNewRoute(std::string nodeId,
        std::string roadId) {
...
void TraCIScenarioManager::commandChangeRoute(std::string nodeId,
        std::string roadId, double travelTime) {
    if (travelTime >= 0) {
        ...
        TraCIBuffer buf = queryTraCI(CMD_SET_VEHICLE_VARIABLE,
                TraCIBuffer() << variableId << nodeId << variableType << count
                        << edgeIdT << edgeId << newTimeT << newTime);
        ASSERT(buf.eof());
    } else {
        uint8_t variableId = VAR_EDGE_TRAVELTIME;
        uint8_t variableType = TYPE_COMPOUND;
...
double TraCIScenarioManager::commandDistanceRequest(Coord position1,
        Coord position2, bool returnDrivingDistance) {
    ...
    // query road network boundaries
    TraCIBuffer buf = queryTraCI(CMD_GET_SIM_VARIABLE,
            TraCIBuffer() << variable << simId << variableType << count << p1
                    << p2 << dType);
    uint8_t cmdLength_resp;
    buf >> cmdLength_resp;
    uint8_t commandId_resp;
...
Coord TraCIScenarioManager::commandPositionConversion(Coord position) {
    ...
    // query road network boundaries
    TraCIBuffer buf = queryTraCI(CMD_GET_SIM_VARIABLE,
            TraCIBuffer() << variable << simId << variableType << count << p
                    << uByte << pType);
    uint8_t cmdLength_resp;
    buf >> cmdLength_resp;
    uint8_t commandId_resp;
...
void TraCIScenarioManager::commandStopNode(std::string nodeId,
        std::string roadId, double pos, uint8_t laneid, double radius,
        double waittime) {
    ...
    TraCIBuffer buf = queryTraCI(CMD_SET_VEHICLE_VARIABLE,
            TraCIBuffer() << variableId << nodeId << variableType << count
                    << edgeIdT << edgeId << stopPosT << stopPos << stopLaneT
                    << stopLane << durationT << duration);
    ASSERT(buf.eof());
}

void TraCIScenarioManager::commandSetTrafficLightProgram(
        std::string trafficLightId, std::string program) {
    TraCIBuffer buf = queryTraCI(CMD_SET_TL_VARIABLE,
            TraCIBuffer() << static_cast<uint8_t>(TL_PROGRAM) << trafficLightId
                    << static_cast<uint8_t>(TYPE_STRING) << program);
    ASSERT(buf.eof());
}

void TraCIScenarioManager::commandSetTrafficLightPhaseIndex(
        std::string trafficLightId, int32_t index) {
    TraCIBuffer buf = queryTraCI(CMD_SET_TL_VARIABLE,
            TraCIBuffer() << static_cast<uint8_t>(TL_PHASE_INDEX)
...
void TraCIScenarioManager::commandAddPolygon(std::string polyId,
        std::string polyType, const TraCIColor& color, bool filled,
...
void TraCIScenarioManager::commandRemovePolygon(std::string polyId,
        int32_t layer) {
    TraCIBuffer p;
...
void TraCIScenarioManager::commandAddPoi(std::string poiId, std::string poiType,
        const TraCIColor& color, int32_t layer, Coord pos) {
    TraCIBuffer p;
...
bool TraCIScenarioManager::commandAddVehicle(std::string vehicleId,
        std::string vehicleTypeId, std::string routeId, simtime_t emitTime_st,
    ...
    int32_t count = 6;
    int32_t emitTime =
            (emitTime_st < 0) ? (-1) : (floor(emitTime_st.dbl() * 1000));
    TraCIBuffer buf = queryTraCIOptional(CMD_SET_VEHICLE_VARIABLE,
            TraCIBuffer() << variableId << vehicleId << variableType << count
                    ...
                    << (uint8_t) TYPE_DOUBLE << emitSpeed << (uint8_t) TYPE_BYTE
                    << emitLane, success);
    ASSERT(buf.eof());
...
std::string TraCIScenarioManager::genericGetString(uint8_t commandId,
...
Coord TraCIScenarioManager::genericGetCoord(uint8_t commandId,
        std::string objectId, uint8_t variableId, uint8_t responseId) {
...
double TraCIScenarioManager::genericGetDouble(uint8_t commandId,
        std::string objectId, uint8_t variableId, uint8_t responseId) {
...
std::list<std::string> TraCIScenarioManager::genericGetStringList(
        uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId) {
...
std::list<Coord> TraCIScenarioManager::genericGetCoordList(uint8_t commandId,
        std::string objectId, uint8_t variableId, uint8_t responseId) {
...
TraCIScenarioManager::TraCICoord TraCIScenarioManager::omnet2traci(
        Coord coord) const {
